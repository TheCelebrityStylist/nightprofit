...
export default async function AvailabilityPage({params}:{params:Promise<{token:string}>}){
  const {token}=await params;let state:{state?:string;starts_at?:string;ends_at?:string;deadline_at?:string;venue_name?:string;venue_timezone?:string;preferred_language?:"nl"|"en";employee_name?:string;entries?:Array<{starts_at:string;ends_at:string;availability:"available"|"preferred"|"preferably_not"|"unavailable";note?:string|null}>}|null=null;
  ...
  return <AvailabilityResponse token={token} periodStart={state.starts_at} periodEnd={state.ends_at} deadlineAt={state.deadline_at||state.ends_at} venueName={state.venue_name||"Jouw vestiging"} venueTimezone={state.venue_timezone||"UTC"} employeeName={state.employee_name||""} preferredLanguage={state.preferred_language||"nl"} initialEntries={state.entries||[]}/>;
}
