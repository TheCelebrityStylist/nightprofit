...
export default async function AvailabilityPage({params}:{params:Promise<{token:string}>}){
  const {token}=await params;let state:{state?:string;starts_at?:string;ends_at?:string;venue_name?:string}|null=null;
  try{const env=serverEnv();if(!env.NEXT_PUBLIC_SUPABASE_URL||!env.NEXT_PUBLIC_SUPABASE_ANON_KEY)throw new Error("CONFIGURATION_REQUIRED");const supabase=createClient<Database>(env.NEXT_PUBLIC_SUPABASE_URL,env.NEXT_PUBLIC_SUPABASE_ANON_KEY,{auth:{persistSession:false,autoRefreshToken:false}});const {data}=await supabase.rpc("inspect_availability_request",{target_token_hash:hashAvailabilityToken(token)});state=data as typeof state}catch{state={state:"invalid"}}
  ...
  return <AvailabilityResponse token={token} periodStart={state.starts_at} periodEnd={state.ends_at} venueName={state.venue_name||"Jouw vestiging"}/>;
}
